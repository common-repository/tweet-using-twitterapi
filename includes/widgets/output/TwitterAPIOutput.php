	...
	foreach($strings as $string) {
		if($count_posts <= $number_of_posts) {
			...
			if( !empty( $instance['tf_show_profile_thumnails'] ) ) {
				...
				$result.= "<div class='tf_widget_image_thumb_wrap'><img src=". $string['user']['profile_image_url_https'] ." alt='twitter_img_thumb'></div>";
			...
			if( !empty($string['entities']['media']) ) {
				if( !empty( $instance['tf_post_images']) ) {
					$result.= "<div class='tf-widget-media-image'><a class='image-popup-no-margins' href=". $string['entities']['media'][0]['media_url_https'] ."><img src=". $string['entities']['media'][0]['media_url_https'] ." alt='Media Image' width=".$string['entities']['media'][0]['sizes']['medium']['w']." height=".$string['entities']['media'][0]['sizes']['medium']['h']."></a></div>";
				}
			}
