				...
				foreach($strings as $string) {
					if($count_posts <= $number_of_posts) {
						...
						if(get_option('tf_profile_thumbnail')) {
							$result.= "<div class='tf_list_image_thumb_wrap'><img src=". $string['user']['profile_image_url_https'] ." alt='twitter_img_thumb'></div>";
						...
						if( !empty($string['entities']['media']) ) {
							if(get_option('tf_post_thumbmnails')) {
								$result.= "<div class='tf-media-image'><a class='image-popup-no-margins' href=". $string['entities']['media'][0]['media_url_https'] ."><img src=". $string['entities']['media'][0]['media_url_https'] ." alt='Media Image' width=".$string['entities']['media'][0]['sizes']['medium']['w']." height=".$string['entities']['media'][0]['sizes']['medium']['h']."></a></div>";
							}
						}
		...
		static function tf_specific_tweet($atts, $content = null) {
			...
			foreach($strings as $string) {
				if( isset($string['created_at']) ) {
					...
					if( !empty($string['entities']['media']) ) {
						if(get_option('tf_post_thumbmnails')) {
							$result.= "<div class='tf-media-image-single'><a class='image-popup-no-margins' href=". $string['entities']['media'][0]['media_url_https'] ."><img src=". $string['entities']['media'][0]['media_url_https'] ." alt='Media Image' width=".$string['entities']['media'][0]['sizes']['medium']['w']." height=".$string['entities']['media'][0]['sizes']['medium']['h']."></a></div>";
						}
					}
					$result.= "<div class='tf_single_tweet tf_tweets_feeds_".$count++."'>";
					if(get_option('tf_profile_thumbnail')) {
						$result.= "<div class='tf_list_image_thumb_wrap_single'><img src=". $string['user']['profile_image_url_https'] ." alt='twitter_img_thumb'></div>";
					...
					$result.= "<div class='tf_error_generated'>Incorrect Tweet ID format. Please check documentation on how to https://deepenbajracharya.com.np/documentation/twitter/1.0/#tf_tweet_id</div>";
				} 
			}
			echo $result;
