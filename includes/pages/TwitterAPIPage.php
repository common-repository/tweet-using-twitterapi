...
if( !class_exists('TwitterAPIPage') ) {
	...
	class TwitterAPIPage {
		...
		public function tf_twitter_settings() {
			?>
			<h2><?php _e('Twitter API Settings', 'tweet-using-twitterapi'); ?></h2>
			<?php
			if( isset($_REQUEST['publish_settings_tf']) ) {
				...
				if( $_REQUEST['publish_settings_tf'] ) {
					...
					if( $tf_username == null || $tf_oauth_access_token == null || $tf_access_secret_token == null || $tf_consumer_key == null || $tf_consumer_secret_key == null ) {
						?>
						<div class="wrap">
							<div class="notice notice-error"><p><strong><?php _e('Awww !! We hate this red bar. Perhaps its causing because one of the necessary field is blank. :)', 'tweet-using-twitterapi'); ?></p></strong></div>
						</div>
						<?php
					} else {
						if(isset($_REQUEST['tf_action_type'])) {
							...
							<div class="wrap">
								<div class="notice notice-warning notice-custom-tf is-dismissible">
									...
									<p><?php printf( 
										__('<strong>Successfully Updated. Clear the Twitter Cache for fetching new data. <a id="tf-flush-cache" href="%s">Clear</a></strong>', 'tweet-using-twitterapi'),
										$url 
										); ?></p>
								</div>
							</div>
							...
							<div class="wrap">
								<div class="notice notice-success notice-custom-tf is-dismissible">
									<p><?php _e( 'Successfully Added !', 'tweet-using-twitterapi' ); ?></p>
								</div>
							</div>
							<?php
						}
